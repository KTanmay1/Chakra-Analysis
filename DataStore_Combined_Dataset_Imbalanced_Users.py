...
class ImbalancedChakra(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    chakra = db.Column(db.String(50))
...
def store_answers():
    ...
    # Check for imbalanced chakras and add them to the ImbalancedChakra table
    if root_score >= 1:
        db.session.add(ImbalancedChakra(chakra='Root'))
    if sacral_score >= 1:
        db.session.add(ImbalancedChakra(chakra='Sacral'))
    if solar_plexus_score >= 1:
        db.session.add(ImbalancedChakra(chakra='Solar Plexus'))
    if heart_score >= 1:
        db.session.add(ImbalancedChakra(chakra='Heart'))
    if throat_score >= 1:
        db.session.add(ImbalancedChakra(chakra='Throat'))
